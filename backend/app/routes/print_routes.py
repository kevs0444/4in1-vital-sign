...
def format_receipt(data):
    ...
    # Report ID
    report_id = data.get('measurement_id') or data.get('id')
    if not report_id:
        report_id = f"HG{str(int(datetime.now().timestamp()))[-6:]}"
